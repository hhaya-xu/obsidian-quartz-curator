...
> Codex Step 5f 子步骤 —— 自动对比 preview-base.html 新旧版本，提取 CSS 变更，生成 SCSS 补丁。
>
> **用法：** 将下方完整脚本保存为 `style-extraction.ps1`，在 Quartz 项目根目录执行：
>
> ```powershell
> .\style-extraction.ps1 -NewFile "construction-site\preview-base.html"
> ```
>
> 即可自动 diff、映射、输出 patch。

---

## 概述

主公在 html-visual-editor 微调后，伯喈不再靠肉眼 diff，而是运行此脚本自动执行：

1. 读取 `preview-base.html`（新版）与 `preview-base.prev.html`（旧版）
2. diff `<style id="design-css">` 标签内的 CSS
3. 通过 `16-anchors-mapping.md` 将 @zone 映射到 Quartz 选择器
4. 生成符合 `diff-format-spec.md` idempotent 规范的 SCSS 补丁
5. 伯喈审核签字后 Apply
...
如果 Codex 选择逐步骤执行而非运行完整脚本，下面是关键函数对照：

| 步骤 | 函数/操作                        | 注意                                                            |
| ---- | -------------------------------- | --------------------------------------------------------------- |
| 1    | `Test-Path` 验证输入             | 首次运行无旧版时跳过                                            |
| 2    | `Get-DesignCSS`                  | 必须用 `[System.IO.File]` 非 `Get-Content`（防中文乱码）        |
| 3    | `Split-ByZone`                   | 正则含行首锚定 + 22 zone 白名单                                 |
| 4    | `Get-QuartzSelectorMap`          | 从 16-anchors-mapping.md 解析 Markdown 表，补充 6 个结构级 zone |
| 5    | 逐 zone 生成 diff                | idempotent 格式：`-` 含旧 CSS+锚点，`+` 含新 CSS+锚点           |
| 6    | `[System.IO.File]::WriteAllText` | 确保 UTF-8 无 BOM                                               |
...
| 情况                           | 脚本行为                                      |
| ------------------------------ | --------------------------------------------- |
| 无 `preview-base.prev.html`    | 所有 zone 视为新增，`-` 块仅含锚点注释        |
| `design-css` 标签为空          | 静默退出，输出 "无变更"                       |
| @zone 不在白名单               | 被忽略，不会分配为新 zone                     |
| @zone 对应的 Quartz 选择器过期 | 标记 `__UNMAPPED__`，伯喈审核时发现并手动修正 |
| CSS 含中文注释                 | `[System.IO.File]` API 确保不被破坏           |

---

## 验证清单（伯喈审核前）

- [ ] 补丁中所有 Quartz 选择器在 `public/index.html` grep 命中
- [ ] 无 `@import` 语句混入
- [ ] 伪元素全部用单冒号
- [ ] idempotent 格式：`-`/`+` 行均含 `/* @zone:xxx */` 锚点
- [ ] 输出补丁文件编码为 UTF-8 无 BOM

---

_obsidian-quartz-curator · 样式析出规范 · v0.6_
